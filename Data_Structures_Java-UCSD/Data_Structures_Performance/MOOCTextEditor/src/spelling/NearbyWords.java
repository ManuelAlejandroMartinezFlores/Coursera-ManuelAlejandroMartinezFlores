...
public class NearbyWords implements SpellingSuggest {
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i=0; i<sLength;i++){
			...
			sb.append(s.substring(0,pointer));
			if(pointer<sLength-1){
				sb.append(s.substring(pointer+1));
			}
			pointer++;
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && count < numSuggestions) {
			...
			String elem = ((LinkedList<String>) queue).poll();
			List<String> res = distanceOne(elem, false);
			for (String s : res){
				if (!visited.contains(s) && count < numSuggestions) {
					visited.add(s);
					queue.add(s);
					if (dict.isWord(s)) {
						retList.add(s);
