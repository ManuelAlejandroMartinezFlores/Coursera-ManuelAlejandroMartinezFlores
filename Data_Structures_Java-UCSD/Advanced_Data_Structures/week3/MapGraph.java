...
class MapNode implements Comparable {
	...
	public boolean equals(Object o) {
		...
		return this.location.equals(node.getLocation());
	}
	...
	void setActualDistance(double dist) {
		actualDistance = dist;
	}
	...
	MapNode getClosestNeighbor(Set<MapNode> neighbors, MapNode startNode) {
		...
		if (neighbors == null) {
			...
			for (MapNode node : neighbors) {
				edges.add(new MapEdge("demo", this, node));
			}
		}
		for (MapEdge edge : edges) {
...
class MapEdge {
	...
	MapEdge (String name, String type, MapNode n1, MapNode n2, double len){
		roadName = name;
		roadType = type;
		start = n1;
		end = n2;
	...
	MapEdge (String name, MapNode n1, MapNode n2){
		this(name, "", n1, n2, DEFAULT_LENGTH);
	}
	
	MapEdge (String name, String type, MapNode n1, MapNode n2){
		this(name, type, n1, n2, DEFAULT_LENGTH);
	}
